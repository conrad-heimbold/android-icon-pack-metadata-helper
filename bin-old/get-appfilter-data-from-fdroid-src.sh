...
if [ "$1" == "--help" ] || [ "$1" == "-h" ] || [ "$1" == "--usage" ]; then
  ...
  if [ ! -f $1 ] || [ ! -d $2 ]; then
    ...
    # SOURCE_URL_REPO comes from the F-Droid Metadata FILE, after "Repo:"
    # SOURCE_URL_CODE comes from the F-Droid Metadata FILE, after "Source Code:"
    FILE="$1"; DEST_DIR=$2; FILENAME=$(basename $FILE);
    # PART 1: PARSE ALL THE NECESSARY INFORMATION FROM THE METADATA FILES IN F-DROID
    # =============================================================================================================================
    SOURCE_URL_REPO=$(grep "^Repo:" $FILE | sed 's/Repo\://' | sed 's/\.git$//');
    SOURCE_URL_CODE=$(grep "^Source Code:" $FILE | sed 's/Source Code\://');
    # only take the SOURCE_URL* that is not empty.
    # test if both SOURCE_URLS are empty => ERROR
    if [ ! -z "$SOURCE_URL_REPO" ] && [ ! -z "$SOURCE_URL_CODE" ]; then SOURCE_URL="$SOURCE_URL_REPO"; fi
    if [ ! -z "$SOURCE_URL_REPO" ] && [   -z "$SOURCE_URL_CODE" ]; then SOURCE_URL="$SOURCE_URL_REPO"; fi
    if [   -z "$SOURCE_URL_REPO" ] && [ ! -z "$SOURCE_URL_CODE" ]; then SOURCE_URL="$SOURCE_URL_CODE"; fi
    # ERROR: NO SOURCE URL!
    if [   -z "$SOURCE_URL_REPO" ] && [   -z "$SOURCE_URL_CODE" ]; then echo "$FILENAME - ERROR: no source URL specified!" >&2; fi
    ...
    SEARCH_LNCHPD=$(echo $SOURCE_URL | grep "launchpad" )
    ...
    if [ ! -z "$SEARCH_SRCFRG" ] || [ ! -z "$SEARCH_GOOGLE" ]; then
      REPO_NAME=$(basename $(dirname                   $SOURCE_URL_CODE));
    ONLINE_SERVICE_URL=$(dirname $(dirname $(dirname $SOURCE_URL_CODE))); fi
    # Get the app name
    ...
    # URL FORMAT: 
    # GIT     ON GITHUB: https://github.com/$DEVELOPER/$REPO_NAME/raw/{$COMMIT or HEAD or $TAG or $BRANCH or master}/$PATH_TO_FILE/$FILENAME
    # GIT     ON GITLAB: https://gitlab.com/$DEVELOPER/$REPO_NAME/raw/{$COMMIT or HEAD or $TAG or $BRANCH or master}/$PATH_TO_FILE/$FILENAME
    # GIT     ON BTBCKT: https://bitbucket.org/$DEVELOPER/$REPO_NAME/raw/{$COMMIT or HEAD or $TAG or $BRANCH or master}/$PATH_TO_FILE/$FILENAME
    # HG      ON BTBCKT: https://bitbucket.org/$DEVELOPER/$REPO_NAME/raw/{$COMMIT or HEAD or $TAG or $BRANCH or tip}/$PATH_TO_FILE/$FILENAME
    # GIT     ON SRCFRG: https://sourceforge.net/p/$REPO_NAME/code/ci/{$COMMIT or HEAD or $TAG or $BRANCH or master}/tree/$PATH_TO_FILE/$FILENAME?format=raw
    # GIT-SVN ON SRCFRG: https://sourceforge.net/p/$REPO_NAME/svn/{$COMMIT or HEAD or $TAG or $BRANCH or master}/tree/$PATH_TO_FILE/$FILENAME?format=raw
    # GIT-SVN ON SRCFRG: https://sourceforge.net/p/$REPO_NAME/code/{$COMMIT or HEAD}/tree/trunk/$PATH_TO_FILE/$FILENAME?format=raw
    # GIT-SVN ON SRCFRG: http://svn.code.sf.net/p/$REPO_NAME/code/trunk/$PATH_TO_FILE/$FILENAME
    # HG      ON SRCFRG: http://hg.code.sf.net/p/$REPO_NAME/code/raw-file/{$COMMIT or tip or $TAG or $MASTER_BRANCH}/$PATH_TO_FILE/$FILENAME
    # BZR     ON LNCHPD: 
    #          VIEW_URL="https://bazaar.launchpad.net/~$DEVELOPER/$REPO_NAME/{$BRANCH or trunk}/view/head:$PATH_TO_FILE/$FILENAME"
    #      DOWNLOAD_URL=$(wget -q "$VIEW_URL" --output-document=/dev/stdout | grep "download file" | sed 's|<a href="||' | sed 's|">download file</a>||')
    ...
    echo "   SOURCE_URL_REPO:      $SOURCE_URL_REPO" 							>> $DEST_DIR/$APP_NAME_SIMPLE/METADATA.txt
    echo "   SOURCE_URL_CODE:      $SOURCE_URL_CODE" 							>> $DEST_DIR/$APP_NAME_SIMPLE/METADATA.txt
